...
def date_wav_filename(ENF_Year, ENF_Month, ENF_date_Begin, ENF_date_End, ENF_Hour_Begin, ENF_Hour_End):
    # ################  时间判断
    begin_time_str = ENF_Year + '-' + ENF_Month + '-' + ENF_date_Begin  # 字符串表示的开始时间
    ...
    begin_week_str = get_week_str(begin_week_int)  # 星期的字符串
    end_week_str = get_week_str(end_week_int)

    ENF_Year_int = int(ENF_Year)  # 日期表示为数字形式
    ...
    month_str = get_month_str(ENF_Month_int)  # 月份的字符串
    ...
    if ENF_date_Begin_int == ENF_date_End_int:
        ...
        for ii in range(Num_of_File):
            ...
            newHour = '%02d' % newHour
            storeFileStrings = storeFileStrings + ENF_Year + '_' + month_str + '_' + ENF_date_str + '_' + end_week_str + '_'+ newHour + '_00_00'    #######23个字符一个文件名的数据（不加.wav后缀）
    # ## within two days
    ...
    # ## within more than two days
        ...
        for ii in range(Num_of_Full_Days + 2):
            ...
            week_str = get_week_str(week_int)  # 星期的字符串
            WeekDayStrings = WeekDayStrings + week_str  #######三个字符一个星期的数据
